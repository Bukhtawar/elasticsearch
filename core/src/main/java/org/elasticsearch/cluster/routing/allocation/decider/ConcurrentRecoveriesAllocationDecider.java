...
public class ConcurrentRecoveriesAllocationDecider extends AllocationDecider {
    ...
    public static final String NAME = "concurrent_recoveries";
    ...
    public Decision canRemain(RoutingAllocation allocation) {
        if (clusterConcurrentRecoveries == -1) {
            return allocation.decision(Decision.NO, NAME, "undefined cluster concurrent recoveries");
        }
        int relocatingShards = allocation.routingNodes().getRelocatingShardCount();
        if (relocatingShards >= clusterConcurrentRecoveries) {
            return allocation.decision(Decision.YES, NAME,
                "too many shards are concurrently relocating [%d], limit: [%d]",
                relocatingShards, clusterConcurrentRecoveries);
        }
