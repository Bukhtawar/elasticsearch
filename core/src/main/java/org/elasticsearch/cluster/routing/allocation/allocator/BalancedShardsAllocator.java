    ...
    public static class Balancer {
        ...
        public void moveShards() {
            ...
            for (Iterator<Iterator<ShardRouting>> it = allocation.routingNodes().nodeShardIterator(); it.hasNext();) {
                ...
                if (allocation.deciders().canRemain(allocation) == Decision.YES) {
                    logger.trace("skipping relocation as the cluster level limit has reached");
                    return;
                }
                Decision decision = allocation.deciders().canMoveAway(shardRouting, allocation);
