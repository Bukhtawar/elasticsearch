...
public class AllocationBenchmark {
    ...
    @Param({
     // indices| shards| replicas| nodes| source| target | concurrentRecoveries
       /* "       10|      2|        0|       1|      1|      1|",
        "       10|      3|        0|       1|      1|      2|",
        "       10|     10|        0|       1|      1|      5|",
        "      100|      1|        0|       1|      1|     10|",
    ...
    private Logger LOG = LogManager.getLogger(AllocationBenchmark.class);
    ...
    public void setUp() throws Exception {
        ...
        if(sourceNodes > targetNodes) {
            clusterConcurrentRecoveries = targetNodes*concurrentRecoveries;
        }else {
            clusterConcurrentRecoveries = sourceNodes*concurrentRecoveries;
        }
        clusterExcludeStrategy = Allocators.createAllocationService(
        Settings.builder().put("cluster.routing.allocation.node_concurrent_recoveries", String.valueOf(concurrentRecoveries))
        .put(ConcurrentRecoveriesAllocationDecider.CLUSTER_ROUTING_ALLOCATION_CLUSTER_CONCURRENT_RECOVERIES_SETTING.getKey(),
        String.valueOf(clusterConcurrentRecoveries)).put("cluster.routing.allocation.exclude.tag", "tag_1").build());
        ...
        verifyConcurrentRecoveries(clusterState);
    }

    private void verifyConcurrentRecoveries(ClusterState clusterState) {
        LOG.error("Relocating shards :{}", clusterState.getRoutingNodes().getRelocatingShardCount());
        if (sourceNodes > targetNodes) {
            for (int i = 1; i <= targetNodes; i++) {
                assert (concurrentRecoveries == clusterState.getRoutingNodes().getIncomingRecoveries("node_t_" + i));
            }
        } else {
            for (int i = 1; i <= sourceNodes; i++) {
                assert (concurrentRecoveries == clusterState.getRoutingNodes().getOutgoingRecoveries("node_s_" + i));
            }
        }
    }
   ...
   // @Benchmark
    public ClusterState measureExclusionOnZoneUnawareStartedShard() throws Exception {
